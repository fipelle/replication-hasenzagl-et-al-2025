...
# ----------------------------------------------------------------------------------------------------------------------
    ...
    d    = zeros(n+nR);
    d[2] = 1;
    Z    = [[ones(4); 0; ones(n-5)] [zeros(1,3); ones(3,3); zeros(1,3); ones(n-5,3)] [zeros(4); ones(n-4)] zeros(n) [zeros(5); 1 ./ σʸ[end-3:end]]];

    # Other loadings
    Z1 = permutedims(kron(Matrix(I,3,3), [1, 0, 1]));                                                                                     # idio C, idio C+, idio trend
    Z2 = permutedims(kron(Matrix(I,3,3), [1, 0]));                                                                                        # idio C, idio C+
    Z  = [Z cat(dims=[1,2], [1 0 1 ./ σʸ[1]], [1.0 0.0], Z1, Z2, [1.0 0.0 1.0])];
    ...
    # EA
    if estim_bool == true
        Z_ind[9, 7] .= true; # EC expectations -> T
    end
    ...
    T = cat(dims=[1,2], T_c_ext, [T_ct for i=1:2]..., T_c, [T_ct for i=1:3]..., [T_c for i=1:3]..., T_ct, zeros(2*nQ, 2*nQ));
    Q = cat(dims=[1,2], Q_c_ext, [T_ct for i=1:2]..., T_c, [T_ct for i=1:3]..., [T_c for i=1:3]..., T_ct, zeros(2*nQ, 2*nQ));
    ...
    P̄¹    = cat(dims=[1,2], zeros(4,4), [P̄_ct for i=1:2]..., P̄_c, [P̄_ct for i=1:3]..., [P̄_c for i=1:3]..., P̄_ct, zeros(2*nQ, 2*nQ));
    ...
    λ     = vcat([1; zeros(3)], [λ_ct for i=1:2]..., λ_c, [λ_ct for i=1:3]..., [λ_c for i=1:3]..., λ_ct, zeros(2*nQ));
    ρ     = copy(λ);
    λ_ind = λ .!= 0;
    ρ_ind = copy(λ_ind);
