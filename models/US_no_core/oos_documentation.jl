...
function load_data(nyears)
    ...
    for v=1:size(data_vintages,3)
        ...
        for i=1:n
            ...
            start_ind_i = size(raw_data_vintage_i_v,1)-h+1;
            for t=start_ind_i:-1:1

                if ismissing(raw_data_vintage_i_v[t]) == false
                    start_ind_i += 1;
                    break;
                else
                    start_ind_i -= 1;
...
# User input
data_path = "./data/US.xlsx";

# Run documentation
point_forecasts, rw_forecasts, outturn, data_vintages, date, h, n, chunk0,
       monthly_gdp, output_gap, BC_clean, EP_clean, BC, EP, T_INFL = load_data(15);
...
unique_releases = unique_releases[year.(unique_releases) .>= 2005];
...
# Create charts
splots = Array{Any,1}(undef, 9);
titles = ["GDP", "GDP SPF", "URATE", "EMPL", "OIL", "INFL", "CORE INFL", "INFL SPF", "EXP INFL"];
gr()
...
df_core_fc = DataFrame([collect(1:36) point_forecasts[:,7,:]]);
rename!(df_core_fc, vcat(:fc_horizon, df_output_gap_names[2:end]));
FileIO.save("./results_csv/forecast_core.csv", df_core_fc);
...
df_core_outturn = DataFrame([collect(1:36) outturn[:,7,:]]);
rename!(df_core_outturn, vcat(:fc_horizon, df_output_gap_names[2:end]));
FileIO.save("./results_csv/outturn_core.csv", df_core_outturn);
