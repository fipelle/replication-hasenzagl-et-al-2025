...
Plots.savefig(p1, "$(model_folder)/img/rmsfe_plot.pdf");

# Save to csv
FileIO.save("$(model_folder)/results_csv/msfe_tc.csv", DataFrame(TC_SE));
FileIO.save("$(model_folder)/results_csv/msfe_rw.csv", DataFrame(RW_SE));
...
Plots.savefig(p2, "$(model_folder)/img/output_gap.pdf");
...
FileIO.save("$(model_folder)/results_csv/output_gap.csv", df_output_gap);
...
potential_output_yoy = log.(potential_output[13:end,:])-log.(potential_output[1:end-12,:]);
...
Plots.savefig(p3, "$(model_folder)/img/potential_output.pdf");
...
FileIO.save("$(model_folder)/results_csv/potential_output.csv", df_potential_output);
...
FileIO.save("$(model_folder)/results_csv/forecast_headline.csv", df_headline_fc);
...
FileIO.save("$(model_folder)/results_csv/outturn_headline.csv", df_headline_outturn);
...
for hz=1:h
    ...
    Plots.savefig(p_hz, "$(model_folder)/img/headline_forecast_h$(hz).pdf");
end
